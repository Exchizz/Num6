...
Doub rtbis(T &func, const Doub x1, const Doub x2, const Doub xacc) {
	...
	for (Int j=0;j<JMAX;j++) {
		...
		double dk = xmid - last_xmid;
		last_xmid = xmid;
		std::cout << j << "\t" << xmid << "\t" << dk << std::endl;
...
Doub rtflsp(T &func, const Doub x1, const Doub x2, const Doub xacc) {
	...
	for (Int j=0;j<MAXIT;j++) {
		...
		double dk = rtf - last_rtf;
		last_rtf = rtf;
		std::cout << j << " \t" << rtf << " \t" << dk << std::endl;
		Doub f=func(rtf);
		if (f < 0.0) {
			del=xl-rtf;
			xl=rtf;
...
Doub rtsec(T &func, const Doub x1, const Doub x2, const Doub xacc) {
	...
	for (Int j=0;j<MAXIT;j++) {
		...
		fl=f;
		rts += dx;
		double dk = rts - last_rts;
		last_rts = rts;
		std::cout << j << "\t" << rts << "\t" << dk << std::endl;
		f=func(rts);
...
Doub zriddr(T &func, const Doub x1, const Doub x2, const Doub xacc) {
	...
	if ((fl > 0.0 && fh < 0.0) || (fl < 0.0 && fh > 0.0)) {
		...
		for (Int j=0;j<MAXIT;j++) {
			...
			double dk = xm - last_xm;
			last_xm = xm;
			std::cout << j << "\t" << xm << "\t" << dk << std::endl;
...
Doub rtnewt(T &funcd, const Doub x1, const Doub x2, const Doub xacc) {
	...
	for (Int j=0;j<JMAX;j++) {
		...
		double dk = rtn-rtn_last;
		rtn_last = rtn;
		std::cout << j << "\t" << rtn << "\t" << dk << std::endl;
